...
const DB = ([] as IDataType[]).prefillRandNums(pagenationInfo.totalCount).map((_x, i) => {
  let _id = uid()
  return new DataType({ id: _id, name: { ar: `نوع${i}`, en: `type${i}`, id: _id } })
})
const server = {
  list: (query: any) => {
  ...
  post: (payload: { data: IDataType }) => {
    payload
    return new Promise<TResult<DataType>>((res, _rej) => {
      const newItem = new DataType({ ...payload.data })
      DB.push(newItem)
      res({ data: DB })
    })
  },
  update: (payload: { data: IDataType }) => {
    return new Promise<TResult<DataType>>((res, _rej) => {
      const newItem = new DataType({ ...payload.data })
