<template>
  ...
  <div class="d-flex justify-content-center mb-3">
    <button class="btn btn-primary ml-4" :class="{ disabled: isLastPage || isBusy }" @click="changeText">{{ readyText }}</button>
  </div>
</template>
...
const readyText = ref('Change My Caption')
const changeText = () => {
  readyText.value = dataService.demoMethod('khaled', 'Obaid')
}
</script>
